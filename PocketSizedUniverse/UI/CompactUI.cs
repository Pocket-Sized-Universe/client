...
public class CompactUi : WindowMediatorSubscriberBase
{
        ...
        : base(logger, mediator, "###PocketSizedUniverseMainUI", performanceCollectorService)
    {
        ...
        WindowName = $"Pocket Sized Universe {dev} ({ver.Major}.{ver.Minor}.{ver.Build})###PocketSizedUniverseMainUI";
        Toggle();
#else
        var ver = Assembly.GetExecutingAssembly().GetName().Version;
        WindowName = "Pocket Sized Universe " + ver.Major + "." + ver.Minor + "." + ver.Build + "###PocketSizedUniverseMainUI";
#endif
    ...
    protected override void DrawInternal()
    {
        ...
        if (!_apiController.IsCurrentVersion)
        {
            ...
            UiSharedService.ColorTextWrapped($"Your Pocket Sized Universe installation is out of date, the current version is {ver.Major}.{ver.Minor}.{ver.Build}. " +
                $"It is highly recommended to keep Pocket Sized Universe up to date. Open /xlplugins and update the plugin.", ImGuiColors.DalamudRed);
        }
    ...
    private string GetServerError()
    {
        return _apiController.ServerState switch
        {
            ...
            ServerState.Disconnected => "You are currently disconnected from the Pocket Sized Universe server.",
            ServerState.Disconnecting => "Disconnecting from the server",
            ServerState.Unauthorized => "Server Response: " + _apiController.AuthFailureMessage,
            ServerState.Offline => "Your selected Pocket Sized Universe server is currently offline.",
            ...
            ServerState.NoAutoLogon => "This character has automatic login into PSU disabled. Press the connect button to connect to PSU.",
            _ => string.Empty
        };
    }
